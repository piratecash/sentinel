...
def has_dash_conf():
    ...
    # configured, including JSONRPC access in piratecash.conf
    try:
...
def main():
    ...
    if not has_required_env_vars() and not has_dash_conf():
        print("DashCore must be installed and configured, including JSONRPC access in piratecash.conf")
        sys.exit(1)

    # deprecation warning
    if not has_required_env_vars() and has_dash_conf():
        print("deprecation warning: JSONRPC credentials should now be set using environment variables. Using piratecash.conf will be deprecated in the near future.")
