...
def get_dash_conf():
    if sys.platform == 'win32':
        dash_conf = os.path.join(os.getenv('APPDATA'), "PirateCore/piratecash.conf")
    else:
        home = os.environ.get('HOME')

        dash_conf = os.path.join(home, ".piratecore/piratecash.conf")
        if sys.platform == 'darwin':
            dash_conf = os.path.join(home, "Library/Application Support/PirateCore/piratecash.conf")
