...
class DashConfig():
    ...
    def slurp_config_file(self, filename):
        # read piratecash.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is piratecash.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data):
        # get rpc info from piratecash.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # determine default rpc port from testnet= setting in piratecash.conf
        network = 'mainnet'
        ...
        default_port = 19606 if (network == 'mainnet') else 19696

        # use default port for network if not specified in piratecash.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from piratecash.conf
